...
// 2. Importar modelos y el *Controlador*
import type { Game, Player } from '../../interfaces/game.models';
import { initGameController, handleLoadGame, handleStartGame } from '../../controllers/gameController';
 ...
 */
const createPlayersColumn = (): [HTMLElement, (game: Game) => void, (isDisabled: boolean) => void] => {
  ...
  const updatePlayersColumn = (game: Game) => {
    header.innerHTML = `<span>${game.players.length}/15</span> Jugadores`;
...
export const renderWaitingRoom = (container: HTMLDivElement, gameId: string) => {
  ...
  const [playersColumn, updatePlayersView, disableStartButton] = createPlayersColumn();
  const chatColumn = createChatColumn();
  
  main.appendChild(playersColumn);
  main.appendChild(chatColumn);

  roomContainer.appendChild(header);
  roomContainer.appendChild(globalMessage);
  roomContainer.appendChild(main);
  ...
  const startButton = container.querySelector('#start-game-button') as HTMLButtonElement;
  if (startButton) {
