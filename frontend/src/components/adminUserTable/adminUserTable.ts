...
export class AdminUsersTableComponent {
    ...
    async render() {
        this.container.innerHTML = '<div class="loading-spinner">Cargando usuarios...</div>'
        ...
        if (!users) {
            this.container.innerHTML = '<p class="error-msg">No se pudieron cargar los usuarios.</p>'
            return
        }
    ...
    private renderTableBody(users: User[]) {
        ...
        if (users.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" class="no-results">No se encontraron usuarios</td></tr>'
            return
        }
        ...
        users.forEach(user => {
            ...
            const rolesHtml = user.roles 
                ? user.roles.map(r => `<span class="role-badge ${r.name}">${r.name}</span>`).join('') 
            ...
            if (isEditing) {
                ...
                const btnSave = tr.querySelector('.btn-save') as HTMLButtonElement
                const btnCancel = tr.querySelector('.btn-cancel') as HTMLButtonElement
    ...
    private async saveUser(userId: number) {
        const nickInput = document.getElementById(`edit-nick-${userId}`) as HTMLInputElement
        const emailInput = document.getElementById(`edit-email-${userId}`) as HTMLInputElement
        ...
        const success = await userController.updateUser(userId, { nickname, email })
    ...
    private async handleDeleteUser(user: User) {
        const confirmed = confirm(`¿Eliminar a "${user.nickname}"?\n\nEsta acción no se puede deshacer.`)
