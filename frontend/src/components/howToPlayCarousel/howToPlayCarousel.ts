...
export class Carousel {
    ...
    constructor(containerID: string, slidesData: ISlideData[], initialIndex: number = 0) {
        const container = document.getElementById(containerID);
        if (!container) {
            throw new Error(`No se pudo encontrar el contenedor con el ID ${containerID}`)
        }
    ...
    private initDOM(): void {
        ...
        this.paginationContainerElement.className = 'carousel-pagination-container';
    ...
    private updateView(): void {
        ...
        this.currentSlideComponent = new Slide(this.slidesData[this.currentIndex]);
        this.slideDisplayElement.innerHTML = '';//Limpia el contenedor
        this.slideDisplayElement.appendChild(this.currentSlideComponent.getElement());
        ...
        this.paginationContainerElement.appendChild(this.paginationComponent.render());
    ...
    public goToSlide(index: number): void {
        ...
        if (index === this.currentIndex || index < 0 || index >= this.slidesData.length) {
            return;
        }
